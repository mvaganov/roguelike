...
/// <summary>
/// found at http://www.adammil.net/blog/v125_roguelike_vision_algorithms.html
/// </summary>
public abstract class RoguelikeVisibility
{
...
public sealed class MilazzoVisibility : RoguelikeVisibility
{
	...
	/// </param>
	public MilazzoVisibility(Func<Coord, bool> blocksLight, Action<Coord> setVisible, Func<Coord, int> getDistance)
	{
	...
	void Compute(int octant, Coord origin, int rangeLimit, int x, Slope top, Slope bottom)
	{
		...
		for (; x <= (int)rangeLimit; x++) // (x <= (int)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
		{
			...
			{
				if (rangeLimit < 0 || GetDistance(new Coord(x, y)) <= rangeLimit) // skip the tile if it's out of visual range
				{
	...
	bool BlocksLight(int x, int y, int octant, Coord origin)
	{
		...
		return _blocksLight(new Coord(nx, ny));
	}
	...
	void SetVisible(int x, int y, int octant, Coord origin)
	{
		...
		_setVisible(new Coord(nx, ny));
	}

	readonly Func<Coord, bool> _blocksLight;
	readonly Func<Coord, int> GetDistance;
	readonly Action<Coord> _setVisible;
}

public sealed class RayCastVisibility : RoguelikeVisibility
{
	...
	/// </param>
	public RayCastVisibility(Coord mapSize, Func<int, int, bool> blocksLight, Action<int, int> setVisible,
	...
	public override void Compute(Coord origin, int rangeLimit)
	{
		SetVisible(origin.col, origin.row);
		if (rangeLimit != 0)
		{
			Rect area = new Rect(0, 0, MapSize.col, MapSize.row); // cast to the edge of the map by default
	...
	void TraceLine(Coord origin, int x2, int y2, int rangeLimit)
	{
